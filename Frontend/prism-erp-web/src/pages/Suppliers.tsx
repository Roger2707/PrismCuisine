...
import type { SupplierDto, CreateSupplierRequest, UpdateSupplierRequest } from '../services/types/purchasing.types';
...
export default function Suppliers() {
  ...
  const openEdit = (item: SupplierDto) => {
    setEditing(item);
    setForm({
      code: item.code,
      name: item.name,
  ...
  const handleSave = async () => {
    try {
      if (editing) {
        ...
        await suppliersApi.update(editing.id, update);
      ...
      <div className="page-content">
        <div className="data-table-container">
          <div className="table-header">
            <h2>Supplier List</h2>
            <button className="add-button" onClick={openCreate}>+ Add Supplier</button>
          </div>
